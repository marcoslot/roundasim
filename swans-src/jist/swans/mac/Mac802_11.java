...
public class Mac802_11 implements MacInterface.Mac802_11
{
	 ...
	 */
	public Mac802_11(int id, Clock vehicleClock, Scheduler vehicleScheduler, RadioInfo radioInfo)
	{
		identity = id;
		clock = vehicleClock;
		...
		localAddr = new MacAddress(identity);
		...
		packetNextHop = null;
		// proxy
		self = this;// (MacInterface.Mac802_11)JistAPI.proxy(this, MacInterface.Mac802_11.class);
	}
	...
	private void doDifs()
	{
		if(isRadioIdle())
		{
			if(waitingNav())
			{
				startTimer(nav-clock.getNanos(), MAC_MODE_SNAV);
			}
			else
			{
	...
	private void sendRts()
	{
		...
		radioEntity.transmit(rts, delay, duration, onRtsDone(delay + duration));
		// wait for EOT, schedule CTS wait timer

	}

	private Runnable onRtsDone(final long delay) {
		return new Runnable() {
			public void run() {


				scheduler.scheduleForOwner(
						new Runnable() {
							public void run() {
	...
	{
		// create cts packet
		final MacMessage.Cts cts = new MacMessage.Cts(rts.getSrc(), (int)(
				rts.getDuration() 
		...
		radioEntity.transmit(cts, delay, duration, scheduleDeferred(onCtsDone(cts), delay + duration));

	}

	private Runnable onCtsDone(final MacMessage.Cts cts) {
		return new Runnable() {
			public void run() {
	...
	private void sendDataBroadcast()
	{
		...
		radioEntity.transmit(data, delay, duration, scheduleDeferred(onCfDone(true,true), delay + duration));
	...
	private void sendDataUnicast(boolean afterCts)
	{
		...
		radioEntity.transmit(data, delay, duration, scheduleDeferred(onUnicastDone, delay + duration));
		// wait for EOT, and schedule ACK timer

	}

	Runnable onUnicastDone = new Runnable() {
		public void run() {
	...
	private void sendAck(MacMessage.Data data)
	{
		...
		radioEntity.transmit(ack, delay, duration, scheduleDeferred(onCfDone(false,false), delay + duration));
	}

	private Runnable onCfDone(final boolean backoff, final boolean delPacket) { 
	    return new Runnable() {
	        public void run() {
	            // wait for EOT, check for outgoing packet
	            self.cfDone(backoff, delPacket);
	        }
	    };
	}
	...
	{
		long currentTime = clock.getNanos();
		...
		if(nav2 > this.nav)
		{
			...
			if (isRadioIdle() && hasPacket())
			{
				...
				// Only do this for ns-2 comparison.
				//startTimer(nav - currentTime, MAC_MODE_SNAV);
			}
		}
	...
	public void startTimer(long delay, byte mode)
	{
		...
		final int keepTimerId = this.timerId;

		scheduler.scheduleForOwner(
				new Runnable() {
					public void run() {
						self.timeout(keepTimerId);
					}
				},
				identity,
				clock.getNanos() + delay
	...
	private Runnable scheduleDeferred(final Runnable task, final long delay) {
		return new Runnable() {
			public void run() {
				scheduler.scheduleForOwner(task, identity, clock.getNanos() + delay);
			}
		};
	}
